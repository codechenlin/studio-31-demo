...
import { Globe, ArrowRight, Copy, Check, ShieldCheck, Search, AlertTriangle, KeyRound, Server as ServerIcon, AtSign, Mail, TestTube2, CheckCircle, Dna, DatabaseZap, Workflow, Lock } from 'lucide-react';
...
export function SmtpConnectionModal({ isOpen, onOpenChange }: SmtpConnectionModalProps) {
        ...
        case 3:
            return (
                <div className="space-y-4">
                    ...
                    <Form {...form}>
                        <form id="smtp-form" onSubmit={form.handleSubmit(onSubmitSmtp)} className="space-y-4">
  ...
  const renderRightPanelContent = () => {
    return (
        <div className="relative p-6 border-l h-full flex flex-col justify-center items-center text-center bg-muted/20">
             <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-primary/5 via-transparent to-transparent opacity-50"/>
             <div className="absolute top-2 right-2 flex items-center gap-1 text-xs text-muted-foreground"><div className="size-2 rounded-full bg-green-400 animate-pulse"/>Estado del Proceso</div>
            
            <AnimatePresence mode="wait">
            {currentStep === 2 && verificationStatus === 'pending' && (
                <motion.div key="pending-check" {...cardAnimation}>
                     <p className="text-muted-foreground">Esperando acción del usuario...</p>
            ...
            {currentStep === 3 && (
                 <motion.div key="step3-actions" {...cardAnimation} className="w-full space-y-4">
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl p-0 grid grid-cols-1 md:grid-cols-3 gap-0 h-[600px]">
