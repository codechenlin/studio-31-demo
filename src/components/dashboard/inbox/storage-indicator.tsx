...
export function StorageIndicator({ used, total, gradientColors, hoverGradientColors, onClick }: StorageIndicatorProps) {
    ...
    const hoverStyle = hoverGradientColors 
        ? { '--hover-from': hoverGradientColors[0], '--hover-to': hoverGradientColors[1] } as React.CSSProperties
        : {};

    return (
        <div 
            className="group p-0.5 rounded-lg bg-transparent hover:bg-gradient-to-br from-[var(--hover-from)] to-[var(--hover-to)] transition-all duration-300"
            style={hoverStyle}
        >
            <button 
                onClick={onClick}
                className="w-64 h-24 rounded-md bg-card/80 border border-border/50 backdrop-blur-sm p-4 flex items-center gap-4 transition-all duration-300 group-hover:shadow-xl group-hover:-translate-y-1"
