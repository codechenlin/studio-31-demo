...
export function BimiVmcStatusModal({ isOpen, onOpenChange, email, senderEmail }: BimiVmcStatusModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl bg-zinc-900/90 backdrop-blur-xl border border-cyan-400/20 text-white overflow-hidden" showCloseButton={false}>
                ...
                <DialogHeader className="z-10 text-center flex flex-col items-center pt-8">
                     <motion.div
                        initial={{ scale: 0, opacity: 0 }}
                        animate={{ scale: 1, opacity: 1 }}
                        transition={{ delay: 0.1, type: "spring", stiffness: 150 }}
                     >
                        <Avatar className="size-32 border-4 border-cyan-400/30 shadow-[0_0_20px_#00ADEC]">
                            <AvatarImage src={email.avatarUrl} alt={email.from}/>
                            <AvatarFallback className="text-5xl font-bold bg-gradient-to-br from-blue-400 to-cyan-400 text-white">
                            {senderInitial}
                            </AvatarFallback>
                        </Avatar>
                     </motion.div>
                     <DialogTitle className="text-2xl font-bold mt-4 text-white">{email.from}</DialogTitle>
                     <DialogDescription className="font-mono text-base text-cyan-300/80">{senderEmail}</DialogDescription>
                </DialogHeader>

                <div className="py-6 z-10 space-y-4">
                    <StatusIndicator 
                        status={!!email.bimi}
                        title="Verificación BIMI"
                        description="BIMI (Brand Indicators for Message Identification) permite mostrar tu logo verificado junto a tus correos, aumentando la confianza y el reconocimiento de marca."
