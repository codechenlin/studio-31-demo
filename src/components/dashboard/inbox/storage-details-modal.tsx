...
const CircularChart = ({ percentage, total, used, last30Days }: { percentage: number, total: number, used: number, last30Days?: number }) => {
    const isMainChart = last30Days === undefined;
        ...
        <div className="relative w-48 h-48">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                <span className="text-3xl font-bold font-mono text-white leading-none">
                    {(used / 1024).toFixed(2)}
                    <span className="text-lg font-sans text-white/70 ml-1">GB</span>
                </span>
                {isMainChart ? (
                    <span className="text-sm text-white/50">de {(total / 1024).toFixed(0)} GB</span>
                ) : (
                    <span className="text-sm text-white/50">Últimos 30 días</span>
                )}
            </div>
        </div>
    );
...
export function StorageDetailsModal({ isOpen, onOpenChange }: { isOpen: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-6xl w-full h-[650px] flex flex-col p-0 gap-0 bg-black/80 backdrop-blur-xl border-2 border-[#AD00EC]/30 text-white overflow-hidden">
                <style>{`
                    @keyframes grid-pan { 0% { background-position: 0% 0%; } 100% { background-position: 100% 100%; } }
                    .animated-grid { background-image: linear-gradient(#AD00EC 1px, transparent 1px), linear-gradient(to right, #AD00EC 1px, transparent 1px); background-size: 3rem 3rem; animation: grid-pan 60s linear infinite; }
                ...
                <div className="flex items-center justify-between p-4 border-b border-[#AD00EC]/20 shrink-0 bg-black/50">
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-px bg-[#AD00EC]/20 overflow-hidden">
                    {storageData.sections.map(section => (
                        <div key={section.id} className="bg-black/30 p-6 flex flex-col">
                ...
                <div className="p-6 border-t border-[#AD00EC]/20 bg-black/50 grid grid-cols-1 md:grid-cols-3 items-center gap-6">
                    <div className="md:col-span-2 flex justify-around items-center">
                        <div className="flex flex-col items-center gap-2">
                             <CircularChart percentage={(totalUsed / storageData.total) * 100} total={storageData.total} used={totalUsed}/>
                             <p className="text-sm font-semibold text-white/90">Total Usado</p>
                        </div>
                        <div className="flex flex-col items-center gap-2">
                            <CircularChart percentage={(4500 / totalUsed) * 100} total={totalUsed} used={4500} last30Days/>
                             <p className="text-sm font-semibold text-white/90">Uso Reciente</p>
                        </div>
                    </div>
