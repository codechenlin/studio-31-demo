...
import { checkApiHealth as checkVmcApiHealth } from '@/ai/flows/api-health-check-flow';
...
export async function checkApiHealthAction() {
  try {
    const result = await checkVmcApiHealth();
    if (result.status === 'ok') {
        return { success: true, data: result };
    }
    return { success: false, error: `API returned status: ${result.status}` };
  } catch (error: any) {
    console.error('VMC API health check action error:', error);
